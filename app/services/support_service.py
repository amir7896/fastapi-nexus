...
class SupportService:
    ...
    def list_conversations(
        ...
        *,
        ...
        is_staff = current_user.role is UserRole.ADMIN
    ...
    def unread_count(self, *, current_user: User) -> SupportUnreadResponse:
        count = self._conversations.unread_count(
    ...
    def start_conversation(
        ...
        *,
        ...
        if current_user.role is UserRole.ADMIN:
            raise BadRequestError("Admins reply in existing conversations")
        order_id = None
        product_id = None
    ...
    def _require_conversation(self, conversation_id: UUID, current_user: User) -> SupportConversation:
        ...
        if current_user.role is not UserRole.ADMIN and conversation.user_id != current_user.id:
    ...
    def _acknowledge(self, conversation: SupportConversation, current_user: User) -> None:
        ...
        seen = self._conversations.mark_messages_seen(conversation.id, reader_is_staff=is_staff)
        if not seen:
            return
        notify_support_seen(
            customer_id=conversation.user_id,
    ...
    def _to_conversation(
        ...
        *,
        ...
        is_staff = viewer.role is UserRole.ADMIN
        if unread is None:
        ...
        return SupportConversationRead(
            ...
            customer_name=conversation.user.name if conversation.user else "Customer",
            customer_email=conversation.user.email if conversation.user else None,
